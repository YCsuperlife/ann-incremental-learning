...
class MultiLayerPerceptron:
    ...
    def fit(self, X, t, learning_rate=0.1, epochs=10000, xtest=None, ytest=None):
        ...
        for k in range(epochs):
            ...
            # (中間-出力)の重みgW2の修正量を求める.(alpha * error * hiddenUnit)
            gW2 = -1.0 * learning_rate * np.dot(delta2.T, z) * m2
            # gW2にはバイアス項も入ってしまっているため、バイアス項の行を取り除く.
            #print gW2.shape
            gW2 = np.delete(gW2, 0, 1)
            #print gW2.shape
            sum_col = np.sum(np.abs(gW2), axis=0)
            ...
            self.hiddenValue += self.alpha * ( sum_col * (-e)  - self.hiddenValue)
...
#        y = self.act2(np.dot(self.weight2 * 0.5, z))
    ...
    def makeMask(self):
        ...
        # 上位50%を使用する.(バイアス項を除く)
        upperNum = int((self.numHidden-1) * 0.5)
        # 0の1次元ベクトルを作成
