...
if __name__ == "__main__":
    ...
    for loop in range(aveLoop):
        ...
        # テストデータを用いて予測精度を計算
        predictions = []
        for i in range(X_test_without4.shape[0]):
            o = mlp.predict(X_test_without4[i])
            predictions.append(np.argmax(o))
        print confusion_matrix(y_test_without4, predictions)
        print classification_report(y_test_without4, predictions)#target_names = target_names)
        ...
        for i in range(X_test_without4.shape[0]):
            ...
            predictions.append(np.argmax(o))
        print confusion_matrix(y_test_without4, predictions)
        print classification_report(y_test_without4, predictions)

        for i in range(len(mlp.hiddenCount)):
            print str(i) , ":", str(mlp.hiddenCount[i])
